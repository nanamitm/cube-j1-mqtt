...
2. `production_tool/wpa_supplicant.conf` を編集して、Wi-Fi の SSID とパスワードを設定する。
   （SSID・パスワードが分からない場合や、あとからスマホで設定したい場合は**編集せずそのまま**で構いません。詳細は[スマホだけで Wi-Fi を設定する](#スマホだけで-wi-fi-を設定する初回セットアップ)を参照してください。）
...
## スマホだけで Wi-Fi を設定する（初回セットアップ）

自宅 Wi-Fi の SSID・パスワードが分からない場合や、USB メモリを編集せずに設定したい場合は、`production_tool/wpa_supplicant.conf` を**同梱のまま（`ssid="ssid"` のプレースホルダのまま）**でセットアップできます。

Cube J1 が標準で備えている Wi-Fi Direct アクセスポイント（`CubeJ-xxxxxx`）経由で設定画面にアクセスし、そこから自宅 Wi-Fi を設定します。

### 手順

1. `wpa_supplicant.conf` を編集せずに USB メモリでセットアップし、電源を入れる。
2. セットアップ完了後（LED が白色に10回点滅）、スマホの Wi-Fi 設定から **`CubeJ-xxxxxx`** に接続する（パスフレーズ: **`12345678`**）。
3. 多くの端末では「インターネットに接続していません」という通知やログイン画面が自動的に表示されます。表示されない場合はブラウザで **`http://192.168.100.1/`** を開いてください。
4. ユーザー名・パスワード（初期値 `admin` / `cubej1`）を入力する。
5. 自宅 Wi-Fi の SSID とパスワードを入力して「Connect」を押す。
6. 接続に成功すると LED が緑色に点灯し、**セットアップ用アクセスポイントは自動的に停止**します。以後は `http://<Cube J1 の IP アドレス>:8080/` から通常の設定画面を利用してください。

> [!WARNING]
> 接続できる Wi-Fi は **2.4GHz 帯の SSID のみ**です。5GHz 専用の SSID には接続できません。

### セットアップ用アクセスポイントの制限

このアクセスポイントのパスフレーズ **`12345678` は全個体共通の固定値で、公開されています**。つまり電波の届く範囲にいる第三者は誰でも接続できます。そのため以下の制限を設けています。

| 項目 | 挙動 |
|---|---|
| 公開範囲 | **Wi-Fi 設定フォームのみ**。ステータス・設定（Bルート認証情報／MQTTパスワード）・ログ・OTA は `403` を返し、このAP経由では一切参照できません |
| 認証 | Wi-Fi 設定フォームにも Basic 認証が必要です。フォーム自体は秘密を含みませんが、無認証にすると第三者が本機を自分のアクセスポイントに接続させ、そのネットワーク上で設定画面全体（Bルート認証情報を含む）に到達できてしまうためです |
| 有効期間 | **自宅 Wi-Fi が未設定のときだけ**有効です。接続に成功した時点で停止し、以後の起動でも停止したままになります |
| 待受ポート | セットアップ中のみ、本体標準の nginx を停止してポート `80` を使用します。Wi-Fi 設定完了後は nginx を復帰させます |

> [!NOTE]
> ポート `80` を使うのは、`http://192.168.100.1:8080/` の `:8080` を付け忘れると本体標準の nginx が `404` を返し、「サーバは応答しているのにページが無い」という紛らわしい状態になるためです（実際に検証中に発生しました）。
> また、セットアップ中は DNS も本機が応答するようになり、スマホのキャプティブポータル検出（`/generate_204` 等）に応答します。これがないと「インターネットなし」と判定され、通信がモバイル回線側にルーティングされて設定画面に到達できないことがあります。

### Wi-Fi 設定をあとから変更する

自宅 Wi-Fi 接続後は、Web UI の **Config → Wi-Fi** から SSID・パスワードを変更できます。

> [!CAUTION]
> 誤った認証情報を保存すると本機はネットワークから到達できなくなります。その場合は次回起動時にセットアップ用アクセスポイントが再び有効になるので、上記の手順で設定し直してください。それでも復旧しない場合は USB メモリでの再セットアップが必要です。
...
> [!NOTE]
> OTAが更新できるのは `/data/local/` 配下の**既存ファイルの内容**のみで、`/system/etc/init/` への新規initサービスの登録はできません。
> v1.1.5 より前にUSBインストールし、それ以降OTAのみで更新してきた機体は `disable_p2p_ap` サービス自体が存在しないため、OTAで `disable_p2p_ap.sh` を受け取っても工場出荷時P2P/AP（`CubeJ-xxxxxx`）は無効化されません。有効にするにはUSBメモリでの再インストールが必要です（新規インストールであれば最初から正しく登録されるため、この問題は発生しません）。
...
7. **工場出荷時 P2P/AP の制御**: Wi-Fi Direct/P2P AP（`CubeJ-xxxxxx`）を `disable_p2p_ap.sh` で制御します（`disable_p2p_ap` サービスとして毎回の起動時に判定）。自宅 Wi-Fi が設定済みなら停止、未設定ならセットアップ用APとして残します。このAPのWPA2パスフレーズは **`12345678` という固定値**であり、常時発信したままにすると電波の届く範囲の第三者が本体のWeb UI（Bルート認証情報とMQTTパスワードを含む）やADB（ポート`5555`）へ到達できてしまうため、不要になった時点で停止します。詳細は[スマホだけで Wi-Fi を設定する](#スマホだけで-wi-fi-を設定する初回セットアップ)を参照
